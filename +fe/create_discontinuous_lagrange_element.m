function element = create_discontinuous_lagrange_element(dim, order)
    % Create instance of discontinuous Lagrange element.
    %
    % SYNTAX
    %   element = create_discontinuous_lagrange_element(dim, order)
    %
    ...
    switch order
    case 0
        element = fe.create_p0_element(dim);
    otherwise
        element = create_discontinuous_lagrange_element_(dim, order);
    end
end


function element = create_discontinuous_lagrange_element_(dim, order)
    ...
    % Discontinuize dofmap (all dofs are associated with cell)
    for d = 0:dim-1
        num_entities_per_dim = size(entity_dofs{d+1}, 2);
        entity_dofs{d+1} = reshape([], 0, num_entities_per_dim);
    end
    entity_dofs{dim+1} = (1:dof_cntr).';
    ...
    element = fe.FiniteElement('DiscontinuousLagrange', 'affine', order, ...
